<template>
  <div>
    ...
    <h1 class="mb-8 text-3xl font-bold">Permissions</h1>
    <div class="flex items-center justify-between mb-6">
      <search v-model="form.search" class="mr-4 w-full max-w-md" @reset="reset">
      </search>
      <div class="flex justify-end">
      <Link class="btn-indigo" href="/admin/permissions/create">
        <span>Create</span>
        <span class="hidden md:inline">&nbsp;permission</span>
      </Link>
      <Link class="btn-indigo ml-1.5" href="/admin/permissions">
        <span>Refresh</span>
    ...
    <div class="bg-white rounded-md shadow overflow-x-auto">
      <table class="w-full whitespace-nowrap">
        ...
        <tr v-for="permission in permissions" :key="permission.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          ...
          <td class="border-t">
            <Link class="flex items-center px-6 py-4" :href="`/admin/permissions/${permission.id}/edit`" tabindex="-1">
              {{ permission.created_at }}
            </Link>
          </td>
          <td class="w-px border-t">
            <Link class="flex items-center px-4" :href="`/admin/permissions/${permission.id}/edit`" tabindex="-1">
  ...
  watch: {
    form: {
      ...
      handler: throttle(function () {
        this.$inertia.get('/admin/permissions', pickBy(this.form), { preserveState: true })
      }, 150),
    },
  },
  methods: {
