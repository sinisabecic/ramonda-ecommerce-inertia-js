<template>
  <div>
    ...
    <div class="flex justify-start mb-8 max-w-3xl">
      ...
      <img v-if="user.photo" v-for="img in user.photo" class="block ml-4 w-8 h-8 rounded-full" :src="`${user.avatarPath}/${user.id}/${img.url}`" />
    </div>
    ...
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="update">
        ...
        <div class="flex items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
          <button v-if="!user.deleted_at" class="text-orange-600 hover:underline" tabindex="-1" type="button" @click="destroy">Delete User</button>
          <button class="text-indigo-700 hover:underline" tabindex="-1" type="button" @click="remove">Remove User</button>
